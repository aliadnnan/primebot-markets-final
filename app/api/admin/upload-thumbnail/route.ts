 ...
 * on Vercel. The upload form advertises 5 MB, so any realistic thumbnail went out
...
async function createSignedUpload(request: NextRequest) {
  ...
  if (contentType && !ALLOWED_IMAGE_TYPES.includes(contentType)) {
    return NextResponse.json(
      { success: false, error: `Invalid file type. Allowed types: ${ALLOWED_IMAGE_TYPES.join(', ')}` },
      { status: 400 }
    )
  }

  if (size && size > MAX_THUMBNAIL_SIZE) {
    return NextResponse.json(
      { success: false, error: `File size exceeds maximum of ${MAX_THUMBNAIL_SIZE / 1024 / 1024}MB` },
      { status: 400 }
    )
  }
  ...
  const filename = `thumbnails/${Date.now()}-${Math.random().toString(36).substring(2, 8)}${
    extension ? `.${extension}` : ''
  }`
