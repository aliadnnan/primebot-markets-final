...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body?.account_holder_name !== undefined) {
    update.account_holder_name = String(body.account_holder_name).trim() || null
  }
  if (body?.qr_code_url !== undefined) {
    update.qr_code_url = String(body.qr_code_url).trim() || null
  }
  ...
  if (
    error &&
    /is_active|display_order|updated_at|account_holder_name|qr_code_url/.test(error.message || '')
  ) {
    const reduced = { ...update }
    delete reduced.is_active
    delete reduced.display_order
    delete reduced.account_holder_name
    delete reduced.qr_code_url
